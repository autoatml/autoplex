...
def qe_params_from_config(config: dict):
    ...
    if not os.path.exists(params["fname_pwi_template"]):
        raise ValueError(
            f"Reference QE input file '{params['fname_pwi_template']}' not found."
        )
...
def lock_input(pwi_fname, worker_id):
    ...
    if os.path.exists(pwo_fname):
        return pwi_lock_fname, pwo_fname  # If exists, skip to next pwi
...
class QEstaticLabelling(Maker):
    ...
    qe_run_cmd: str | None = (
        None  # String with the command to run QE (including its executable path/or application name)
    )
    fname_pwi_template: str | None = (
        None  # Path to file containing the template computational parameters
    )
    fname_structures: str | list[str] | None = (
        None  # Path or list[Path] to ASE-readible file containing the structures to be computed
    )
    num_qe_workers: int | None = None  # Number of workers to use for the calculations.
    kspace_resolution: float | None = (
        None  # K-space resolution in Angstrom^-1, used to set the K-points in the pwi file
    )
    koffset: list[bool] = field(
    ...
    def make(self):
        ...
        if (
            self.num_qe_workers is None
        ):  # 1 worker per structure (all DFT jobs in parallel)
    ...
    def check_pwi_template(self, fname_template):
        ...
        if idx_nat_line == 0:  # nat not defined, assume nat = 0
            raise ValueError(
                "Number of atoms line not defined in the template file. Please define 'nat =' in the template file."
            )
        tmp_pwi_lines[idx_nat_line] = "nat = \n"

        # Cancel lines with ATOMIC_POSITIONS and CELL_PARAMETERS
        if (idx_pos_line == 0 and idx_cell_line > 0) or (
            idx_pos_line > 0 and idx_cell_line == 0
        ):
    ...
    def write_pwi(
        ...
        # Check pwi lines
        idx_diskio, idx_outdir, idx_nat_line, idx_kpoints_line, nat = (
            0,
            0,
            0,
            0,
        ...
        for idx, line in enumerate(pwi_template):
            if "nat =" in line:
                idx_nat_line = idx
            elif "disk_io" in line:
                idx_diskio = idx
            elif "outdir" in line:
                idx_outdir = idx
            elif "K_POINTS" in line:
                idx_kpoints_line = idx
        ...
        if (
            idx_diskio == 0 or "none" not in pwi_template[idx_diskio]
        ):  # disk_io is not 'none' (QE default is low for scf)
        ...
        cell_lines += [
            f"{structure.cell[i, 0]:.10f} {structure.cell[i, 1]:.10f} {structure.cell[i, 2]:.10f}\n"
            for i in range(3)
        ]
        ...
        for i, atom in enumerate(structure):
            pos_lines.append(
                f"{atom.symbol} {structure.positions[i, 0]:.10f} {structure.positions[i, 1]:.10f} {structure.positions[i, 2]:.10f}\n"
            )
        ...
        with open(fname_pwi_output, "w") as f:
            for (
                line
            ) in pwi_template:  # Write reference pwi lines (computational parameters)
                f.write(line)
    ...
    def _set_Kpoints(
        ...
        if idx_kpoints_line == 0:
            if (
                Kspace_resolution is None
            ):  # K_POINTS line not found and Kspace_resolution is not defined
                raise ValueError(
                    "K_POINTS line not found in the template file. Please define K_POINTS in the template file or provide Kspace_resolution."
                )
            # Find k-points grid using Monkorst-Pack method based on K-space resolution
            # Get real space cell
            cell = atoms.cell
            ...
            Kpoint_line = (
                f"{MP_mesh[0]} {MP_mesh[1]} {MP_mesh[2]}"  # K-points grid line
            )
        ...
        # K_POINTS is defined by user in reference pwi file, keep the line/s
            ...
            if (
                "gamma" in tmp_pwi_lines[idx_kpoints_line]
                or "Gamma" in tmp_pwi_lines[idx_kpoints_line]
            ):  # KPOINT is 1 line
                ...
                "tpiba" in tmp_pwi_lines[idx_kpoints_line]
                or "crystal" in tmp_pwi_lines[idx_kpoints_line]
            ):  # KPOINTS is multiple lines
                num_ks = int(
                    tmp_pwi_lines[idx_kpoints_line + 1].split()[0]
                )  # Get number of k-points
                kpoints_lines = tmp_pwi_lines[
                    idx_kpoints_line : idx_kpoints_line + num_ks + 2
                ]  # Get k-points lines
                del tmp_pwi_lines[idx_kpoints_line:]
            else:
                raise ValueError(
                    f"K_POINTS format: {tmp_pwi_lines[idx_kpoints_line]} is unknown in pwi template file"
                )
    ...
    def _compute_kpoints_grid(self, cell, Kspace_resolution):
        ...
        print(
            f"Computed k-points mesh: {mesh} for K-space resolution: {Kspace_resolution} Angstrom^-1"
        )  # DEBUG
