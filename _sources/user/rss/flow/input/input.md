...
DFT calculations can be carried out with VASP, CASTEP or FHI-aims. We provide below example inputs for all aforementioned DFT codes. All of them can be specified through the python input script.

Since VASP is the default DFT code, its parameters only need to be specified with the `BaseVaspMaker` if you want to override the default settings. If you want to use CASTEP or FHI-aims, you must specify a code-specific `StaticMaker` in the input script.

__(i) Example input script using VASP__
...
__(ii) Example input script using CASTEP__
...
__(iii) Example input script using FHI-aims__

FHI-aims, like Vasp, is already included in atomate2. Thus, to run FHI-aims as a backend, the command needs to be set in atomate2 configuration file as `AIMS_CMD` variable, or exported directly as `ATOMATE2_AIMS_CMD`. Please follow the [FHI-aims atomate2 tutorial](https://workflows-with-atomate2-fhi-aims-club-tutorials-a0acc2efc2fba83.gitlab.io/) to make sure that atomate2 works on resources of your choice. 

After setting atomate2 up, you're good to go with Autoplex. Autoplex RSS Workflow needs as an argument a `StaticMaker` that is capable to produce energies, forces and stress for a given structure. FHI-aims and atomate2 static maker do its best to try and predict run parameters that return converged result, but there are a few parameters that __must be set__ manually. 

Let's look at the minimal example of `AimsStaticMaker` initialization:
```python
...
rss_job = RssMaker(name="Si_rss", 
                   ...
                   static_energy_maker=# code-specific static maker, see above
                   static_energy_maker_isolated_atoms=# code-specific static maker, see above
                   ).make()
```            
